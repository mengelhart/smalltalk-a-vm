...
static STA_OOP make_array(uint32_t count) {
    STA_ObjHeader *h = sta_heap_alloc(g_heap, STA_CLS_ARRAY, count);
    assert(h);
...
static STA_OOP make_bytearray(uint32_t byte_count) {
    ...
    STA_OOP cls = sta_class_table_get(g_ct, STA_CLS_BYTEARRAY);
    STA_OOP args[] = { cls, STA_SMALLINT_OOP((intptr_t)byte_count) };
    STA_OOP result;
    int rc = sta_primitives[32](args, 1, &result);
    assert(rc == 0);
    return result;
}
...
static STA_OOP make_string(uint32_t byte_count) {
    STA_OOP cls = sta_class_table_get(g_ct, STA_CLS_STRING);
    STA_OOP args[] = { cls, STA_SMALLINT_OOP((intptr_t)byte_count) };
    STA_OOP result;
    int rc = sta_primitives[32](args, 1, &result);
    assert(rc == 0);
...
static STA_OOP make_association(STA_OOP key, STA_OOP value) {
    STA_ObjHeader *h = sta_heap_alloc(g_heap, STA_CLS_ASSOCIATION, 2);
    assert(h);
    sta_payload(h)[0] = key;
...
static void test_basic_at_array(void) {
    ...
    assert(sta_primitives[33](args, 1, &result) == 0);
...
static void test_basic_at_bounds(void) {
    ...
    assert(sta_primitives[33](args, 1, &result) == 2);
...
static void test_basic_at_put_array(void) {
    ...
    assert(sta_primitives[34](args, 2, &result) == 0);
    assert(result == STA_SMALLINT_OOP(99));

    /* Read back */
    STA_OOP args_at[] = { arr, STA_SMALLINT_OOP(2) };
    assert(sta_primitives[33](args_at, 1, &result) == 0);
    assert(result == STA_SMALLINT_OOP(99));
    printf(" ok\n");
}
...
static void test_basic_at_bytearray(void) {
    ...
    assert(sta_primitives[34](args, 2, &result) == 5);
...
static void test_basic_at_nonindexable(void) {
    ...
    assert(sta_primitives[33](args, 1, &result) == 1); /* fail: not indexable */
    printf(" ok\n");
}
...
static void test_basic_at_put_immutable(void) {
    ...
    STA_ObjHeader *h = sta_immutable_alloc(g_sp, STA_CLS_ARRAY, 3);
    assert(h);
    STA_OOP arr = (STA_OOP)(uintptr_t)h;
    STA_OOP result;

    STA_OOP args[] = { arr, STA_SMALLINT_OOP(1), STA_SMALLINT_OOP(99) };
    assert(sta_primitives[34](args, 2, &result) == 4); /* fail: immutable */
    printf(" ok\n");
}
...
static void test_basic_at_arg_not_smallint(void) {
    ...
    assert(sta_primitives[33](args, 1, &result) == 3); /* fail: arg not SmallInt */
    printf(" ok\n");
}
...
static void test_basic_size_array(void) {
    ...
    STA_OOP args[] = { arr };
    assert(sta_primitives[35](args, 0, &result) == 0);
...
static void test_hash_smallint(void) {
    ...
    assert(sta_primitives[36](args, 0, &result) == 0);
...
static void test_identity_hash_stable(void) {
    ...
    STA_OOP args[] = { arr };
    assert(sta_primitives[40](args, 0, &result1) == 0);
    assert(sta_primitives[40](args, 0, &result2) == 0);
...
static void test_identity_hash_distribution(void) {
    ...
    for (int i = 0; i < N_OBJS; i++) {
        ...
        STA_OOP args[] = { arr };
        assert(sta_primitives[40](args, 0, &hashes[i]) == 0);
    }
...
static void test_inst_var_at_association(void) {
    ...
    /* instVarAt: 1 → key */
    STA_OOP args[] = { assoc, STA_SMALLINT_OOP(1) };
    assert(sta_primitives[38](args, 1, &result) == 0);
    assert(result == key);
...
static void test_inst_var_at_put_roundtrip(void) {
    ...
    assert(sta_primitives[39](args, 2, &result) == 0);
    assert(result == STA_SMALLINT_OOP(77));

    /* Read back. */
    STA_OOP args_at[] = { assoc, STA_SMALLINT_OOP(2) };
    assert(sta_primitives[38](args_at, 1, &result) == 0);
    assert(result == STA_SMALLINT_OOP(77));
    printf(" ok\n");
}
...
static void test_inst_var_at_bounds(void) {
    ...
    assert(sta_primitives[38](args, 1, &result) == 2);
...
static void test_inst_var_at_put_immutable(void) {
    ...
    STA_ObjHeader *h = sta_immutable_alloc(g_sp, STA_CLS_ASSOCIATION, 2);
    assert(h);
    STA_OOP assoc = (STA_OOP)(uintptr_t)h;
    STA_OOP result;

    STA_OOP args[] = { assoc, STA_SMALLINT_OOP(1), STA_SMALLINT_OOP(99) };
    assert(sta_primitives[39](args, 2, &result) == 4);
    printf(" ok\n");
}
...
static void test_become_associations(void) {
    ...
    STA_OOP args[] = { a, b };
    assert(sta_primitives[37](args, 1, &result) == 0);

    /* After become:, the OOP 'a' now sees (3, 4) and 'b' sees (1, 2). */
...
static void test_become_immutable_fails(void) {
    ...
    assert(sta_primitives[37](args, 1, &result) == 1);
...
static void test_become_different_size_fails(void) {
    ...
    assert(sta_primitives[37](args, 1, &result) == 6);
    printf(" ok\n");
}
...
static void test_shallow_copy_array(void) {
    ...
    STA_OOP args[] = { arr };
    assert(sta_primitives[41](args, 0, &result) == 0);
    assert(result != arr);  /* not identical */
...
static void test_send_shallow_copy_to_array(void) {
    ...
    STA_OOP result = sta_interpret(slab, g_heap, g_ct, method,
                                    STA_SMALLINT_OOP(0), NULL, 0);
    /* Result is a new array, not identical to the original. */
    assert(result != arr);
...
static void test_send_inst_var_at_to_association(void) {
    ...
    uint8_t bc[] = {
        OP_PUSH_LIT, 0,       /* push assoc */
        OP_PUSH_SMALLINT, 1,  /* push 1 */
        OP_SEND, 1,            /* send #instVarAt: */
        OP_RETURN_TOP, 0
    };
    STA_OOP method = sta_compiled_method_create(g_sp, 0, 0, 0,
        lits, 3, bc, sizeof(bc));
