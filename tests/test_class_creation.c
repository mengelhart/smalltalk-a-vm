...
static STA_OOP sym(const char *name) {
    return sta_symbol_intern(imm, syms, name, strlen(name));
}
...
static STA_OOP create_class_via_prim(STA_OOP superclass,
    ...
    STA_ObjHeader *str_h = sta_heap_alloc(heap, STA_CLS_STRING, words);
    assert(str_h);
    if (len > 0) {
    ...
    STA_ObjHeader *empty_h = sta_heap_alloc(heap, STA_CLS_STRING, 0);
    assert(empty_h);
    ...
    STA_OOP result = 0;
    int rc = sta_primitives[122](prim_args, 5, &result);
...
static void test_class_in_class_table(void) {
    ...
    uint32_t idx = sta_class_table_index_of(ct, point);
    assert(idx != 0);
    assert(sta_class_table_get(ct, idx) == point);
}
...
static void test_new_instance_nil_slots(void) {
    ...
    STA_OOP instance = 0;
    int rc = sta_primitives[31](prim_args, 0, &instance);
    assert(rc == STA_PRIM_SUCCESS);
    assert(instance != 0);
...
static void test_metaclass_wiring(void) {
    ...
    STA_OOP meta = sta_class_table_get(ct, meta_idx);
    assert(meta != 0);
    ...
    STA_OOP obj_meta = sta_class_table_get(ct, obj_h->class_index);
